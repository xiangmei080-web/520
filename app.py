...
# 舊版本可能殘留機車台數=1，首次載入時自動遷移成 0
if "_scooter_default_migrated" not in st.session_state:
    if st.session_state.get("scooter_count", None) == 1:
        st.session_state["scooter_count"] = 0
    st.session_state["_scooter_default_migrated"] = True
...
def build_message(
    ...
    if island_transport == "機車":
        if int(scooter_count or 0) > 0:
            transport_text = f"機車 {int(scooter_count)} 台"
    elif str(island_transport).strip() and island_transport != "無":
        transport_text = island_transport
...
def render_admin_panel() -> None:
    ...
    with col_it_add:
        if st.button("增加", key="add_itinerary_btn", use_container_width=True):
            ...
            if v:
                ...
                st.warning("請輸入行程名稱。")
    with col_it_del:
        del_it = st.selectbox("刪除行程（選擇後按刪除）", options=[""] + list(data["itineraries"]), key="del_itinerary")
    ...
    with col_loc_del:
        del_loc = st.selectbox("刪除報到地點（選擇後按刪除）", options=[""] + list(data["locations"].keys()), key="del_location")
    ...
    del_agency = st.selectbox("刪除旅行社（選擇後按刪除）", options=[""] + list(data["agencies"]), key="del_agency")
    ...
    del_counter = st.selectbox("刪除報到櫃台（選擇後按刪除）", options=[""] + list(data["counters"]), key="del_counter")
...
with col_d:
    ...
    st.markdown(f"<span style='font-size:1rem;font-weight:700'>📆 {sel_date.strftime('%Y/%m/%d')}</span>", unsafe_allow_html=True)

with col_ci:
    st.markdown("🕐 **報到時間**")
    st.caption("自動生成：出發時間 - 30 分（例如 19:30 → 19:00抵達）")
    st.text_input("報到時間（自動：出發時間 - 30 分）", value=st.session_state.get("checkin_auto", ""), disabled=True, label_visibility="visible")

with col_dep:
    st.markdown("🚢 **出發時間**")
    st.caption("手動輸入：19:30 或 1930")
    depart_t = st.text_input("出發時間（手動輸入，例如 19:00 或 1900）", value="09:00", label_visibility="visible", key="depart_manual")
...
counter_choice = st.selectbox("選擇或新增報到櫃台", counter_options, label_visibility="visible", key="counter_choice")
if counter_choice == "（輸入新櫃台）":
    counter = st.text_input("報到櫃台名稱", placeholder="例：櫃台A / 第3櫃台 / 報到中心", label_visibility="visible", key="counter_new")
else:
...
if iti_choice == "（輸入新行程）":
    itinerary = st.text_input("行程名稱", placeholder="例：小琉球半日浮潛", label_visibility="collapsed", key="itinerary_new")
else:
...
passenger_name = st.text_input("旅客姓名", placeholder="例：王小明", label_visibility="visible", key="passenger_name")
st.markdown("</div>", unsafe_allow_html=True)
...
island_transport = st.selectbox("島上交通", ["機車", "導覽車", "無"], key="island_transport")
scooter_count = 0
if island_transport == "機車":
    scooter_count = st.number_input("機車台數", min_value=0, max_value=99, value=0, step=1, key="scooter_count")
st.markdown("</div>", unsafe_allow_html=True)
...
if loc_choice == "（輸入新地點）":
    loc_name = st.text_input("報到地點名稱", placeholder="例：小琉球花瓶岩碼頭", label_visibility="collapsed", key="loc_name_new")
    auto_url = map_url(loc_name) if loc_name else ""
    custom_url = st.text_input("自訂地圖網址（選填，留空自動產生）", value="", placeholder="https://maps.google.com/...", key="loc_custom_url")
    loc_url = custom_url if custom_url else auto_url
    if loc_name and loc_url:
        st.markdown(f"🗺️ <a href='{loc_url}' target='_blank' style='color:#1a6cf5'>{loc_url}</a>", unsafe_allow_html=True)
else:
    loc_name = loc_choice
...
note = st.text_area("備註（可不填）", placeholder="例：請攜帶毛巾、防曬乳……", height=90, label_visibility="collapsed", key="note")
...
if agency_choice == "（輸入旅行社名稱）":
    agency = st.text_input("旅行社名稱", placeholder="例：陽光旅遊", label_visibility="collapsed", key="agency_new")
else:
